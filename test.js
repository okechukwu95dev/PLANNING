...
.platform-data-default-route { margin: 0; font-family: 'Open Sans', Arial, sans-serif; font-weight: 400; background: linear-gradient(180deg, #e0f2ff 0%, #cce9ff 100%); min-height: 100vh; display: flex; flex-direction: column; }
...
/* Card layout */
.platform-data-default-route .cards-container { display: flex; justify-content: center; gap: 30px; flex-wrap: wrap; max-width: 1200px; margin: 0 auto; padding: 0 20px; }
.platform-data-default-route .card { background-color: white; border-radius: 8px; box-shadow: 0 4px 15px rgba(0,0,0,0.05); padding: 30px; display: flex; flex-direction: column; width: calc(50% - 45px); min-width: 300px; margin-bottom: 20px; border-top: 4px solid #0066cc; }
...
@media (max-width: 768px) {
  .platform-data-default-route .cards-container { flex-direction: column; }
  .platform-data-default-route .card { width: 100%; }
}
